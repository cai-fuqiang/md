...
void listen_signal()
{
    ...
    ret = dbus_bus_request_name(connection,"org.freedesktop.login1.Manager",DBUS_NAME_FLAG_REPLACE_EXISTING,&err);
    if(dbus_error_is_set(&err)){
    ...
    dbus_bus_add_match(connection,"type='signal',interface='org.freedesktop.login1.Manager'",&err);
    ...
    while(1){
        ...
        if(dbus_message_is_signal(msg,"org.freedesktop.login1.Manager","PrepareForShutdown") ){
