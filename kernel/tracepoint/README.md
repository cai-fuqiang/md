...
# tracepoint 初始化流程 tracer_init_tracefs
实际上是通过fs_install注册该初始化流程的
代码为:

<!--tracer_init_tracefs BEGIN-->
<details>
<summary><code>tracer_init_tracefs</code></summary>
...
static __init int tracer_init_tracefs(void)
{
    ...
    d_tracer = tracing_init_dentry();	//相当于init /sys/kernel/debug/tracing这个文件
    if (IS_ERR(d_tracer))
        return 0;
...
<!--tracer_init_tracefs END-->
从这个函数来看，该函数主要是初始化和`tracefs`相关的内容。

## tracing_init_dentry

关于这个函数的解释，详情请见:<br/>

[tracing_init_dentry](./tracing_init_dentry.md)
